    ...
    query = """
        WITH ranked_rentabilidade AS (
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    PARTITION BY
                        TRUNC(RAF.DATA_COTACAO),
                        RME.TESOURARIA
                    ORDER BY
                        RAF.COD_REL_APURACAO_FORM DESC
                ) AS rn
            FROM rel_multi_estrategia_cota_nvag RME
            INNER JOIN REL_APURACAO_FORMS RAF
                ON RME.COD_REL_APURACAO_FORM =
                   RAF.COD_REL_APURACAO_FORM
            INNER JOIN REL_APURACAO RA
                ON RAF.COD_REL_APURACAO = RA.CODIGO
            WHERE RME.NIVEL_AGREGACAO =
                  'Rentabilidade Cota Nível Agregação 1'
              AND RA.NOME LIKE
                  '[RESULTADO] FUNDAÇÃO CERES DIARIO OFICIAL#%'
        ),
        ...
        ultima_data AS (
            SELECT
                MAX(TRUNC(DATA_COTACAO)) AS DATA_COTACAO
            FROM ranked_rentabilidade
            WHERE rn = 1
        )
        ...
        LEFT JOIN ranked_posicao R2
            ON TRUNC(R1.DATA_COTACAO) =
               TRUNC(R2.DATA_COTACAO)
           AND R1.TESOURARIA = R2.TESOURARIA
           AND R2.rn = 1
        WHERE R1.rn = 1
          AND TRUNC(R1.DATA_COTACAO) = UD.DATA_COTACAO
        ORDER BY
            R1.TESOURARIA
    """
